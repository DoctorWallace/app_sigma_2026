...
class Command(BaseCommand):
    help = 'Genera propuestas de prueba para todos los usuarios ICTS'
    ...
    def handle(self, *args, **options):
        ...
        # Obtener todos los usuarios ICTS
        icts_users = User.objects.filter(
            groups__name__in=['icts_users', 'revisores', 'responsables', 'managers']
        ).distinct()

        if not icts_users.exists():
            self.stdout.write(self.style.ERROR('No se encontraron usuarios ICTS.'))
            return

        self.stdout.write(f'Generando propuestas para {icts_users.count()} usuarios...')
        ...
        scopes = [
            "El objetivo de este proyecto es caracterizar la microestructura y propiedades de materiales avanzados mediante técnicas de análisis de superficie y volumen. Se utilizarán técnicas de microscopía electrónica de barrido (SEM) y espectroscopía de dispersión de rayos X (EDX) para obtener información detallada sobre la composición y morfología de las muestras.",
            "Este estudio se centra en el análisis de la estructura cristalina y defectos en materiales semiconductores. Se emplearán técnicas de difracción de rayos X y microscopía electrónica de transmisión para determinar la calidad cristalina y la presencia de defectos estructurales.",
            "La investigación tiene como objetivo principal el desarrollo y caracterización de nuevos materiales compuestos con propiedades mejoradas. Se utilizarán técnicas de análisis elemental y morfológico para evaluar la distribución de fases y la integridad de las interfaces.",
            "Este proyecto se enfoca en el estudio de la corrosión y degradación de materiales en condiciones ambientales específicas. Se emplearán técnicas de análisis de superficie para evaluar los mecanismos de corrosión y la efectividad de diferentes tratamientos protectores.",
            "La caracterización de materiales biomédicos requiere un análisis detallado de su biocompatibilidad y propiedades mecánicas. Se utilizarán técnicas de microscopía y espectroscopía para evaluar la interacción con tejidos biológicos."
        ]
        ...
        for user in icts_users:
            # Crear entre 5-8 propuestas por usuario
            num_proposals = random.randint(5, 8)
            ...
            for i in range(num_proposals):
                ...
                # Crear participantes (0-3 por propuesta)
                num_participants = random.randint(0, 3)
                ...
                if status in ['submitted', 'accepted', 'rejected']:
                    self.create_reviews(proposal, status)

                total_created += 1

        self.stdout.write(
            self.style.SUCCESS(
                f'Se crearon {total_created} propuestas de prueba exitosamente.'
            )
        )
    ...
    def create_reviews(self, proposal, status):
        """Crea revisiones para la propuesta"""
        from django.contrib.auth.models import Group
        
        # Obtener revisores
        reviewers_group = Group.objects.filter(name='revisores').first()
        ...
        for reviewer in reviewers:
            ...
            review = ProposalReview.objects.create(
                ...
                reviewer=reviewer,
                decision=decision,
                comments=f"Revisión de {proposal.title} - {random.choice(['Excelente propuesta', 'Buena calidad científica', 'Necesita mejoras', 'Propuesta innovadora'])}",
                **scores
            )
